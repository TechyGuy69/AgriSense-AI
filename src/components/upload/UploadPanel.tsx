...
const UploadPanel = () => {
  ...
  const processData = async () => {
    if (uploadedFiles.length === 0) {
      toast({ title: 'No files to process', description: 'Please upload files first.', variant: 'destructive' });
      return;
    }
    try {
      setProcessing(true);
      setAiResult(null);
      toast({ title: 'Processing started', description: 'Analyzing data with Gemini...' });
      const { data, error } = await supabase.functions.invoke('gemini-analyze', {
        body: { files: uploadedFiles },
      });
      if (error) throw error;
      setAiResult(data as any);
      toast({ title: 'Analysis complete', description: 'AI insights are ready.' });
    } catch (err: any) {
      ...
      {aiResult && (
        <Card className="shadow-card">
          <CardHeader>
            ...
            <CardDescription>Insights generated by Google Gemini</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="prose prose-sm max-w-none dark:prose-invert">
              <pre className="whitespace-pre-wrap text-sm text-foreground">{aiResult.text}</pre>
            </div>
          </CardContent>
        </Card>
      )}
